...
class Healthtip extends Db{
		//ADD HEALTHTIP
	public function add_healthtip($category_id, $healthtips_title, $healthtips_description, $cover_image){
	...
	public function get_category_detail($category_id){
		$sql = "SELECT * FROM healthtips WHERE category_id = ?";
		$stmt = $this->connect()->prepare($sql);
		$stmt->bindParam(1, $category_id, PDO::PARAM_INT);
		$stmt->execute();
...
// $health = new Healthtip();
// $healths = $health->update_healthtip("heloooooo", "hiiiiiii", 93);
// echo $healths;
// echo "<pre>";
// print_r($healths);
// echo "</pre>";
